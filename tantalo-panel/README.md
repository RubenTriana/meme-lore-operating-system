...
2. comprueba que Node.js/npm esté disponible, sin instalar software;
3. inicia el panel y LoreSystem v2 en un servidor Vite local enlazado exclusivamente a `127.0.0.1`;
...
- **No abre el panel:** ejecuta el `.bat` desde la raíz y comprueba que `node --version` y `npm.cmd --version` respondan.
...
La primera prueba cubre presets ligeros y profundos, módulos en espera, aislamiento, vistas, reguladores, filtros, reducción de movimiento, JSON inválido, telemetría no disponible, simulación, costo muy alto, veto y error. La segunda certifica el sistema narrativo y sus límites de canon.
