...
Este manual está dirigido a un autor. Los comandos se ejecutan en PowerShell desde `C:\Users\ruben\OneDrive\Documents\meme_LoreSystem_v2`.
...
```powershell
Set-Location "C:\Users\ruben\OneDrive\Documents\meme_LoreSystem_v2"
npm ci
npm run dev
```

Abre la URL local que imprime Vite. Para comprobar una compilación final usa `npm run build` y después `npm run preview`.
