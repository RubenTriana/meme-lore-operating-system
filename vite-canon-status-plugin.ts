import { updateJsonDocument } from './server/json-store'
...
export function updateBeatStatusDocument(
  ...
  if (!isBeatStatus(change.status))
    throw new Error(`Estado no permitido: ${String(change.status)}.`)
  if (
    !document ||
    typeof document !== 'object' ||
    !Array.isArray((document as CanonDocument).modules)
  ) {
  ...
  const matches = saveTheCat.content.items.filter(
    (entity) => entity.id === change.beatId && entity.type === 'beat',
  )
  if (matches.length !== 1) throw new Error(`No existe un beat único con id ${change.beatId}.`)
  matches[0].status = change.status

  if (!next.metadata || typeof next.metadata !== 'object')
    throw new Error('El master no contiene metadata válida.')
    ...
    configureServer(server) {
      server.middlewares.use('/api/canon/beat-status', async (request, response) => {
        ...
        try {
          ...
          const saved = await updateJsonDocument(masterPath, (current) =>
            updateBeatStatusDocument(current, { beatId: payload.beatId!, status: payload.status! }),
          )
          sendJson(response, 200, {
            universe: saved.document,
            beatId: payload.beatId,
            status: payload.status,
          })
        } catch (error) {
          const message =
            error instanceof Error ? error.message : 'No se pudo actualizar el master.'
