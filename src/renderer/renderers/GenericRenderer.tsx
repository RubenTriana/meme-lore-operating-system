  ...
  const counts = Object.fromEntries([
    ...
    ...novels.map(
      (novel) =>
        [
          novel.id,
          items.filter((item) => getEntityNovelRefs(item).includes(novel.id)).length,
        ] as const,
    ),
  ])
  ...
  return (
    <div className="module-page">
      <header className="module-hero">
        ...
        <div>
          <p className="eyebrow">
            {module.type} · canon {universe.metadata.version}
          </p>
          <h1>{module.title}</h1>
          <p>{module.description ?? 'Módulo gobernado por el canon oficial.'}</p>
        </div>
      </header>
      {items.length > 0 && novels.length > 0 && (
        <NovelNavigator
          novels={novels}
          selected={selection}
          onSelect={setSelection}
          counts={counts}
          includeTransversal={transversalCount > 0}
          label={`${module.title} por novela`}
        />
      )}
        ...
        {visibleItems.map((item) => (
          <Card key={item.id} className="entity-card">
            <div className="card-heading">
              <Badge tone={statusTone(item.status)}>{item.status ?? item.type}</Badge>
              <EntityDetailControl entity={item} index={index} universe={universe} />
            </div>
            <h2>{item.title}</h2>
            <p>{item.summary}</p>
            ...
            {typeof item.development === 'number' && (
              <div className="metric-line">
                ...
                <Progress value={item.development} />
              </div>
            )}
            <div className="reference-line">
              <Link2 size={13} /> {index.references.get(item.id)?.length ?? 0} vínculos canónicos
            </div>
          </Card>
        ))}
      </div>
      {items.length === 0 && (
        <Card className="empty-state">
          <DynamicIcon name={module.icon} size={25} />
          <h2>Sin entradas canónicas</h2>
          <p>Este módulo permanece reservado para futuras expansiones.</p>
        </Card>
      )}
      {items.length > 0 && visibleItems.length === 0 && (
        <Card className="empty-state">
          <DynamicIcon name={module.icon} size={25} />
          <h2>Sin elementos en esta novela</h2>
          <p>El canon no asigna entradas de {module.title} a este volumen.</p>
        </Card>
      )}
    </div>
  )
