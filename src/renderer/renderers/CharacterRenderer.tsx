...
export function CharacterRenderer({ module, universe, index }: RendererProps) {
  const characters = canonicalModuleItems(universe, module)
  ...
  const counts = Object.fromEntries([
    ...
    ...novels.map(
      (novel) =>
        [
          novel.id,
          characters.filter((character) => getEntityNovelRefs(character).includes(novel.id)).length,
        ] as const,
    ),
  ])
  ...
  return (
    <div className="module-page">
      <header className="module-hero">
        <div>
          <p className="eyebrow">Inteligencia de personajes · Tetralogía</p>
          <h1>{module.title}</h1>
          <p>{module.description}</p>
        </div>
        <Badge tone="amber">
          {visibleCharacters.length} de {characters.length} perfiles
        </Badge>
      </header>
      <NovelNavigator
        novels={novels}
        ...
        {visibleCharacters.map((character) => {
          ...
          return (
            <Card className="character-card" key={character.id}>
              ...
              <div className="character-radar">
                <DevelopmentRadar values={scores} />
              </div>
              <div className="character-engine">
                {engineFields.map(
                  ([key, label, Icon]) =>
              ...
              <div className="character-footer">
                <div>
                  <span>Importancia narrativa</span>
                  <Progress value={scores.importance} />
                </div>
                <div>
                  <span>Tiempo narrativo</span>
                  <Progress value={scores.narrativeTime} />
                </div>
                <div>
                  <span>Desarrollo del perfil</span>
                  <Progress value={scores.development} />
                </div>
                <div>
                  <span>Calidad del perfil</span>
                  <Progress value={scores.quality} />
                </div>
                <Link
                  className="button button-secondary character-connections-link"
                  to={`/module/relationships?focus=${encodeURIComponent(character.id)}`}
                  aria-label={`Mostrar ${connectionCount} conexiones de ${character.title}`}
                >
                  <Network size={14} /> Conexiones {connectionCount}
                </Link>
              </div>
            </Card>
          )
