...
export function BeatRenderer({ module, universe, index }: RendererProps) {
  ...
  const changeStatus = async (beatId: string, status: BeatStatus) => {
    ...
    try {
      ...
      setFeedback((current) => ({ ...current, [beatId]: { tone: 'success', text: 'Guardado en master' } }))
    } catch (error) {
  ...
  return (
    <div className="module-page">
      <header className="module-hero">
        <div><p className="eyebrow">Primary narrative engine</p><h1>{module.title}</h1><p>{module.description}</p></div>
        <div className="completion-orb"><strong>{Math.round(global)}%</strong><span>global progress</span></div>
      </header>
      <Card className="beat-summary">
        <div><Sparkles size={18} /><span>Beat coverage</span><strong>{beats.length} / 15</strong></div>
        <div><Flag size={18} /><span>Story progress</span><Progress value={global} /></div>
        <div><CircleDashed size={18} /><span>Missing beats</span><strong>{Math.max(0, 15 - beats.length)}</strong></div>
      </Card>
      <div className="beat-list">
        {beats.map((beat) => {
          const currentStatus = STATUS_OPTIONS.some((option) => option.value === beat.status) ? beat.status as BeatStatus : 'draft'
                ...
                <div className="beat-actions">
                  <label className="beat-status-editor">
                    ...
                    <select
                      ...
                      onChange={(event) => void changeStatus(beat.id, event.target.value as BeatStatus)}
                    >
                      {STATUS_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
                    </select>
                  </label>
                  <EntityDetailControl entity={beat} index={index} className="beat-detail-trigger" />
                </div>
                {feedback[beat.id] && <span className={`beat-save-feedback ${feedback[beat.id].tone}`} role="status">{feedback[beat.id].text}</span>}
              </div>
            </article>
          )
        })}
