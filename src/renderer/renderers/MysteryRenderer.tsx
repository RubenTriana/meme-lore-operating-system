...
export function MysteryRenderer({ module, universe, index }: RendererProps) {
  const mysteries = canonicalModuleItems(universe, module)
  ...
  const counts = Object.fromEntries([
    ...
    ...novels.map(
      (novel) =>
        [
          novel.id,
          mysteries.filter((mystery) => getEntityNovelRefs(mystery).includes(novel.id)).length,
        ] as const,
    ),
  ])
  ...
  return (
    <div className="module-page">
      <header className="module-hero mystery-hero">
        <div>
          <p className="eyebrow">Inteligencia de misterios · Tetralogía</p>
          <h1>{module.title}</h1>
          <p>{module.description}</p>
        </div>
        <Badge tone="amber">
          {visibleMysteries.length} de {mysteries.length} misterios
        </Badge>
      </header>
      <NovelNavigator
        novels={novels}
      ...
      <div className="entity-grid mystery-grid">
        {visibleMysteries.map((mystery) => {
          ...
          return (
            <Card key={mystery.id} className="entity-card mystery-card">
              <div className="card-heading">
                <Badge tone={statusTone(mystery.status)}>{mystery.status ?? mystery.type}</Badge>
                <EntityDetailControl entity={mystery} index={index} universe={universe} />
              </div>
              ...
              {typeof mystery.development === 'number' && (
                <div className="metric-line">
                  <span>Desarrollo</span>
                  <strong>{mystery.development}%</strong>
                  <Progress value={mystery.development} />
                </div>
              )}
              <Link
                className="button button-secondary mystery-connections-link"
                to={`/module/relationships?focus=${encodeURIComponent(mystery.id)}`}
                aria-label={`Mostrar ${connectionCount} conexiones de ${mystery.title}`}
              >
