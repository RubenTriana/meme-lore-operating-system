...
export function ConnectionsPage({ serviceFactory = createAnalysisService }: ConnectionsPageProps) {
  ...
  const filterTypes = useMemo(() => edgeType === 'all' ? [] : [edgeType], [edgeType])
  const neighborhood = useMemo(() => graph && activeId ? connectionNeighborhood(graph, activeId, { maxDepth: depth, maxVisited: 80, edgeTypes: filterTypes }) : undefined, [graph, activeId, depth, filterTypes])
  const directNeighborhood = useMemo(() => graph && activeId ? connectionNeighborhood(graph, activeId, { maxDepth: 1, maxVisited: 80, edgeTypes: filterTypes }) : undefined, [graph, activeId, filterTypes])
