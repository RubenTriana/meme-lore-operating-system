...
const canonReleaseStatus = z.enum([
  'CANON',
  'INHERITED_CANON',
  'CANON_DIRECTION',
  'CANON_CATEGORY',
...
function temporalOrder(
...
): { precision: string; value: number } | undefined {
  ...
  if (precision === 'year')
    return yearPattern.test(value) ? { precision, value: Number(value) } : undefined
  ...
  .superRefine((temporal, context) => {
    if (temporal.end && !temporal.start) {
      context.addIssue({
        ...
        path: ['end'],
        message: 'Temporal end requires a temporal start.',
      })
    }
    ;(['start', 'end'] as const).forEach((field) => {
      ...
      if (value && !temporalValueIsValid(value, temporal.precision)) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: [field],
          message: `Temporal ${field} does not match its precision.`,
        })
      }
    })
    if (temporal.start && temporal.end) {
      ...
      if (start && end && start.precision === end.precision && end.value < start.value) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['end'],
          message: 'Temporal end cannot be earlier than temporal start.',
        })
      }
    }
  })
...
] as const satisfies readonly AnalysisRuleId[]
...
const continuitySchema = z
  .object({
    life: z
      .object({ birth: temporalSchema.optional(), death: temporalSchema.optional() })
      .passthrough()
      .optional(),
    ageAssertions: z
      .array(z.object({ entityRef: id, age: z.number().int().nonnegative() }).passthrough())
      .optional(),
  ...
  .object({
    metadata: z
      .object({
        ...
        version: z
          .string()
          .regex(/^\d+\.\d+\.\d+(-[A-Za-z0-9.-]+)?$/, 'Use semantic versioning, e.g. 0.8.1.'),
        schemaVersion: z
          .string()
          .regex(/^\d+\.\d+(\.\d+)?$/, 'Use a numeric schema version, e.g. 3.2.0.'),
        build: z.string().min(1),
        created: z.string().datetime(),
        updated: z.string().datetime(),
        author: z.string().min(1),
        description: z.string().optional(),
        releaseStatus: canonReleaseStatus.optional(),
        sourceManifest: z.array(z.record(z.string(), z.unknown())).optional(),
        canonStatus: canonReleaseStatus.optional(),
    ...
    if (!entityIds.has(reference)) {
      errors.push(
        issue(
          ...
          'Create the referenced entity or remove this reference.',
        ),
      )
    }
  })
    ...
    return {
      ...
      errors: parsed.error.issues.map((error) =>
        issue(
          ...
          undefined,
          error.message,
          'Correct this field and reload the file.',
        ),
      ),
      warnings: [],
    }
  ...
  data.modules.forEach((module, moduleIndex) => {
    ...
    module.content.items?.forEach((entity, entityIndex) => {
      if (entityIds.has(entity.id)) {
        errors.push(
          issue(
            `modules.${module.id}.items.${entityIndex}.id`,
            entity.id,
            'Duplicate entity id.',
    ...
    module.content.items?.forEach((entity) => {
        ...
        if (knowledgeChangeRefs.has(change.characterRef)) {
          errors.push(
            issue(
              `${path}.characterRef`,
              change.characterRef,
              'Duplicate knowledge change character reference.',
              'Combine changes for the same character.',
            ),
          )
        }
          ...
          if (forgotten.has(knowledge)) {
            errors.push(
              issue(
                path,
                knowledge,
                'Knowledge cannot be learned and forgotten in the same change.',
                'Keep a knowledge value in only one direction.',
              ),
            )
          }
        })
        ...
        if (stateChangeKeys.has(key)) {
          errors.push(
            issue(
              path,
              key,
              'Duplicate state change target.',
              'Keep one state change per entity path.',
            ),
          )
        }
        ...
        if (!entityIds.has(assertion.entityRef)) {
          errors.push(
            issue(
              `${path}.entityRef`,
              assertion.entityRef,
              'Broken age assertion entity reference.',
              'Reference an existing entity.',
            ),
          )
        }
        if (ageAssertionRefs.has(assertion.entityRef)) {
          errors.push(
            issue(
              `${path}.entityRef`,
              assertion.entityRef,
              'Duplicate age assertion entity reference.',
              'Keep one age assertion per entity and event.',
            ),
          )
        }
      ...
      entity.continuity?.exceptions?.forEach((exception, exceptionIndex) => {
        ...
        validateUniqueValues(
          errors,
          `${path}.subjectRefs`,
          exception.subjectRefs ?? [],
          'continuity exception subject reference',
        )
          ...
          if (!entityIds.has(reference)) {
            errors.push(
              issue(
                `${path}.subjectRefs`,
                reference,
                'Broken continuity exception subject reference.',
                'Reference an existing entity.',
              ),
            )
          }
        ...
        validateUniqueValues(
          ...
          `${path}.ruleIds`,
          exception.ruleIds ?? [],
          'continuity exception rule',
        )
      })

      if (!entity.summary) {
        warnings.push({
          path: `entity.${entity.id}.summary`,
          message: 'Entity has no summary.',
          suggestion: 'Add a concise canonical description.',
          severity: 'warning',
        })
      }
    })
