...
import { loadCurrentUniverse, loadUniverse, type LoadResult } from '@/services/universe-loader'
...
export function UniverseProvider({ children }: PropsWithChildren) {
  const initial = useQuery({
    queryKey: ['universe', 'master'],
    queryFn: loadCurrentUniverse,
    staleTime: Infinity,
  })
    ...
    () => ({
      ...
      updateBeatStatus: async (beatId: string, status: BeatStatus) => {
        if (candidateOverride)
          throw new Error('Vuelve al canon base antes de editar el estado de un beat.')
        ...
        if (!response.ok || !payload.universe)
          throw new Error(payload.error ?? 'No se pudo guardar el estado en el master.')
        const result = loadUniverse(payload.universe)
        if (!result.validation.valid)
          throw new Error('El servidor devolvió un master que no supera la validación.')
      ...
      importFile: async (file) => {
        ...
        if (isUniversePatch(payload))
          throw new Error('Los patches deben importarse desde el Centro de propuestas.')
        setBaseOverride(loadUniverse(payload))
      ...
      importPayload: (payload) => {
        if (isUniversePatch(payload))
          throw new Error(
            'La importación de patches está en cuarentena; crea una propuesta antes de simularla.',
          )
        setBaseOverride(loadUniverse(payload))
        setCandidateOverride(null)
        setLoadedAt(performance.now())
      },
      openCandidate: (universe, proposalId) => {
        setCandidateOverride({ proposalId, result: loadUniverse(universe) })
        setLoadedAt(performance.now())
      },
      restoreBase: () => {
        setCandidateOverride(null)
