 ...
 */
export function useVirtualWindow(total: number, rowHeight: number, overscan = 4, scrollOffset = 0) {
  const [viewport, setViewport] = useState({ top: 0, height: typeof window === 'undefined' ? 900 : window.innerHeight })
  useEffect(() => {
    const update = () => setViewport({ top: Math.max(0, window.scrollY - scrollOffset), height: window.innerHeight })
    update()
